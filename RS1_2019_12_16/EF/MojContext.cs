...
namespace RS1_2019_12_16.EF
{
    public class MojContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Komisija>().HasKey(x => new { x.PopravniIspitId, x.NastavnikId });

            modelBuilder.Entity<Komisija>().HasOne(x => x.PopravniIspit).WithMany(x => x.Komisija).HasForeignKey(x => x.PopravniIspitId);
            modelBuilder.Entity<Komisija>().HasOne(x => x.Nastavnik).WithMany(x => x.Komisija).HasForeignKey(x => x.NastavnikId);
